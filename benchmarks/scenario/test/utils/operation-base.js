...
class OperationBase extends WorkloadModuleBase {
  ...
  async initializeWorkloadModule(
    ...
    this.assertSetting("numberOfTxs");

    this.numberOfTxs = this.roundArguments.numberOfTxs;
  }
  ...
  assertConnectorType() {
    ...
    if (!SupportedConnectors.includes(this.connectorType)) {
      throw new Error(
        `Connector type ${this.connectorType} is not supported by the benchmark`
      );
    }
  }
  ...
  assertSetting(settingName) {
    if (!this.roundArguments.hasOwnProperty(settingName)) {
      throw new Error(
        `Test workload error: module setting "${settingName}" is missing from the benchmark configuration file`
      );
    }
  }
